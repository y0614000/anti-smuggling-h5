...
<template>
  <div class="result-overlay" role="presentation">
    <section
      ...
      role="dialog"
      aria-modal="true"
      :aria-label="isSuccess ? '检查完成' : '时间到'"
    >
      <img :src="backgroundImage" alt="" decoding="async" draggable="false" />

      <div class="result-modal__copy">
        <p>{{ isSuccess ? '成功找出3件可疑物品！' : `已找到${foundCount}/3件可疑物品` }}</p>
        <strong>{{ isSuccess ? '获得道具：侦查放大镜' : '还差一点，再试一次吧！' }}</strong>
      </div>
      ...
      <div v-else class="result-modal__failure-actions">
        ...
        <button ref="buttonElement" type="button" aria-label="重新挑战" @click="emit('primary')">
...
.result-modal__failure-actions img {
  display: block;
  width: 100%;
  height: auto;
}

.result-modal__primary:focus-visible {
  border-radius: 999px;
  outline: none;
  filter: brightness(1.04) drop-shadow(0 0 3px rgb(255 192 48 / 70%));
}

.result-modal__failure-actions button:focus-visible {
  border-radius: 999px;
  outline: none;
  filter: brightness(1.04) drop-shadow(0 0 3px rgb(255 192 48 / 70%));
}

.result-modal__primary:active,
.result-modal__failure-actions button:active {
  transform: scale(0.96);
}

@media (prefers-reduced-motion: no-preference) {
  .result-overlay {
    animation: level1-overlay-in 180ms ease-out both;
  }

  .result-modal {
    animation: level1-result-in 420ms cubic-bezier(0.22, 1.25, 0.35, 1) both;
  }
}

@keyframes level1-overlay-in {
  from {
    opacity: 0;
  }
}

@keyframes level1-result-in {
  from {
    opacity: 0;
